...
var header = {
  init: function(){
    ...
    $('#menu').slicknav({
      ...
      closedSymbol: '<span class="icon-right-arrow" aria-haspopup="true" aria-expanded="false"></span>',
      openedSymbol: '<span class="icon-down-arrow" aria-haspopup="true" aria-expanded="true"></span>',
    ...
    //Nav fix if on mobile
    if ($('.top_bar--fixed-true').length){
      var topBarHeight = $('.top_bar--fixed-true').height();
      $('.top_bar--fixed-true.top_bar + .content .header-logo').css('padding-top', topBarHeight + 15);
    }
...
$(function() {
  ...
  $(".dropdown").each(function() {
    if($(this).offset().left - $(window).scrollLeft() > $(window).width()/1.75) {
